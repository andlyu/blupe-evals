"""Eval state machine on the YAM, switched by Quest controller buttons.
...
select. A/X/B/Y are optional direct shortcuts.
  ...
  CONNECT  toggle connect-to-arm. Real i2rt hardware is M2; for now this just flips
           intent and logs -- the arm stays the MuJoCo sim either way.

The POLICY lives on the Jetson and is NOT in this repo: pass
...
Sim is the only working backend now; the SAME state machine + gate graduate to the
real i2rt arm (M2) by swapping SimRobot for a real Robot -- nothing else changes.

Run on the Orin desktop:  DISPLAY=:0 python scripts/eval_yam_states.py
"""
...
N_ARM = 6
EE = "grasp"                       # i2rt tool frame (a body in yam.xml); IK target + FK readout
