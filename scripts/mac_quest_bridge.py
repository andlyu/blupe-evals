"""Mac <-> Quest bridge for operator video, HUD, controller input, and robot actions.

Runs the same gate as eval_states (TELEOP / POLICY / GO_HOME / QUIT, menu + A/X/B/Y),
but headless: instead of an on-screen MuJoCo window it streams video to the Quest. The VIEW
menu option toggles what the headset shows: the robot cameras or the live sim render.
...
os.environ.setdefault("OPENCV_FFMPEG_CAPTURE_OPTIONS", "rw_timeout;5000000|stimeout;5000000")
...
# Teleop, policy, and go-home all share ONE joint-speed cap: E.MAX_VEL (eval_states.py,
    ...
    if judging is not None:                             # policy verdict modal, dead center
        ...
        for i, (lab, col) in enumerate([("SUCCESS", (40, 170, 80)), ("FAIL", (190, 60, 50))]):
            ...
            cv2.putText(rgb, lab, (ox + 16, oy + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (255, 255, 255), 2)
    if connect_arm and link_status != "connected":
        if link_status == "connecting":
            msg = "CONNECTING..."
        elif link_status == "disconnecting":
            msg = "DISCONNECTING..."
        elif link_status == "error":
            msg = "ROBOT CONNECT ERROR"
        elif link_status == "serve_ready":
            msg = "LINKING..." if state in ("TELEOP", "GO_HOME") else "ROBOT ON"
        else:
            msg = "ROBOT OFF"
...
import eval_states as E  # reuse SimRobot/SafeRobot/Console/load_run/_sample_run + constants
from relay_handoff import (policy_start_accepted, release_teleop_link_for_policy,
                           relay_result, can_switch_state, should_attach_teleop_link,
...
def _q_summary(q):
    ...
    if vals.size == 0:
        return "n=0"
    return (
        f"n={vals.size} first={vals[0]:+.3f} last={vals[-1]:+.3f} "
        f"min={float(np.min(vals)):+.3f} max={float(np.max(vals)):+.3f}"
    )
...
class RobotLink:
    """CONNECT: stream the sim joint command to a robot serve endpoint.
    ...
    def connect_async(self):
        ...
        self._stop.clear()
        print(f"[robot-cmd] connect requested {self.host}:{self.port}", flush=True)
        self._thread = threading.Thread(target=self._run, daemon=True)
        self._thread.start()
            ...
            while not self._stop.is_set():
                ...
                if latest is not None:
                    ...
                    with self._wlock:
                        ...
                        f.flush()
                    self._sent_count += 1
                    if self._sent_count <= 3 or now - self._last_cmd_log >= 1.0:
                        print(
                            f"[robot-cmd] sent #{self._sent_count} "
                            f"{_q_summary(latest)} g={grip if grip is not None else '-'}",
                            flush=True,
                        )
    ...
    def close(self, shutdown=False):
        print(
            f"[robot-cmd] close shutdown={shutdown} status={self.status} "
...
class RelayPolicyRunner:
    """Thin wrapper around relay robot controls.

    VR POLICY uses the same relay API as the web UI:
      run  -> /api/cmd?cmd=run_policy
      stop -> /api/cmd?cmd=stop_policy
    VR CONNECT uses the same relay lifecycle command:
      on   -> /api/cmd?cmd=arm_on
      off  -> /api/cmd?cmd=arm_off
    """

    def __init__(self, base_url, token, robot, policy="", timeout=45.0):
        self.base_url = base_url.rstrip("/")
        self.token = token
        self.robot = robot
        self.policy = policy
        self.timeout = timeout
        self.status = "idle"          # idle | starting | running | done | error | stopping
        self.arm_status = "idle"      # idle | connecting | serve_ready | disconnecting | error
        self.last = None
        self.error = None
        self.arm_error = None
        self._stop = threading.Event()
    ...
    def _cmd(self, cmd, timeout=None):
        q = {"token": self.token, "robot": self.robot, "cmd": cmd}
        if cmd == "run_policy" and self.policy:
            q["policy"] = self.policy
        url = f"{self.base_url}/api/cmd?{urllib.parse.urlencode(q)}"
        safe = {k: v for k, v in q.items() if k != "token"}
        started = time.monotonic()
        print(f"[relay-cmd] -> {safe}", flush=True)
        with urllib.request.urlopen(url, timeout=timeout or self.timeout) as resp:
            data = json.loads(resp.read().decode())
        elapsed = time.monotonic() - started
        result = relay_result(data)
        print(
            f"[relay-cmd] <- cmd={cmd} ok={data.get('ok')} "
            f"result={result[:180]!r} dt={elapsed:.2f}s",
            flush=True,
        )
        return data
    ...
    def _run(self):
        try:
            data = self._cmd("run_policy")
            self.last = data
            result = relay_result(data)
            if not policy_start_accepted(data):
                self.status = "error"
                self.error = result or json.dumps(data)[:240]
                if "serve is off" in result:
                    self.arm_status = "error"
                print(f"[relay-policy] start result: {json.dumps(data)[:240]}", flush=True)
                return
            pol = (data.get("data") or {}).get("policy") or self.policy or "default"
            self.status = "running"
            print(f"[relay-policy] {pol} started on {self.robot}", flush=True)
            while not self._stop.wait(0.75):
                st = self._cmd("policy_status")
                self.last = st
                p = (st.get("data") or {})
                if not p.get("running"):
                    self.status = "done"
                    print(f"[relay-policy] {pol} complete", flush=True)
                    return
        except Exception as e:
            self.error = f"{type(e).__name__}: {e}"
            self.status = "error"
            print(f"[relay-policy] error: {self.error}", flush=True)

    def stop(self, wait=False):
        if self._thread is not None and self._thread.is_alive() and self.active:
            self._stop.set()
            self.status = "stopping"
            def _stop_remote():
                try:
                    data = self._cmd("stop_policy", timeout=8.0)
                    self.last = data
                    print(f"[relay-policy] stop result: {json.dumps(data)[:240]}", flush=True)
                except Exception as e:
                    self.error = f"{type(e).__name__}: {e}"
                    print(f"[relay-policy] stop error: {self.error}", flush=True)
            threading.Thread(target=_stop_remote, daemon=True).start()
    ...
    def arm_on(self):
        if self._arm_thread is not None and self._arm_thread.is_alive():
            return
        self.arm_status = "connecting"
        self.arm_error = None
        self._arm_thread = threading.Thread(target=self._arm_cmd, args=("arm_on",), daemon=True)
        self._arm_thread.start()

    def arm_off(self):
        if self._arm_thread is not None and self._arm_thread.is_alive():
            return
        self.arm_status = "disconnecting"
        self.arm_error = None
        self._arm_thread = threading.Thread(target=self._arm_cmd, args=("arm_off",), daemon=True)
        self._arm_thread.start()

    def _arm_cmd(self, cmd):
        try:
            data = self._cmd(cmd, timeout=60.0 if cmd == "arm_on" else 45.0)
            self.last = data
            result = ((data.get("data") or {}).get("result") or data.get("err") or "")
            ok_result = result in ("ON", "already on") or result.startswith("OFF")
            if data.get("ok") and ok_result:
                self.arm_status = "serve_ready" if cmd == "arm_on" else "idle"
                print(f"[relay-arm] {cmd} -> {result}", flush=True)
            else:
                self.arm_status = "error"
                self.arm_error = result or json.dumps(data)[:240]
                print(f"[relay-arm] {cmd} result: {json.dumps(data)[:240]}", flush=True)
        except Exception as e:
            self.arm_status = "error"
            self.arm_error = f"{type(e).__name__}: {e}"
            print(f"[relay-arm] {cmd} error: {self.arm_error}", flush=True)
...
def main(quest_ip: str, port: int = 12345, policy: Optional[str] = None,
         ...
         arm: str = "yam",             # arm profile from scripts/arms.py (yam | so101 | ...)
         direct_serve_control: bool = False,
         relay_policy_url: str = "",
         relay_policy_token: str = "",
         relay_policy_robot: str = "",
         relay_policy: str = ""):
    spec = arms.get(arm)
    E.N_ARM = spec.dof                 # eval_states constants follow the spec: every
    ...
    relay_runner = None
    if relay_policy_url and relay_policy_token and relay_policy_robot:
        relay_runner = RelayPolicyRunner(relay_policy_url, relay_policy_token,
                                         relay_policy_robot, relay_policy)
        run_loop = None
        print(f"[eval] relay policy: {relay_policy or 'default'} on {relay_policy_robot} "
              f"via {relay_policy_url}", flush=True)
    else:
        run_loop = E.load_run(policy) if policy else E._sample_run()
        print(f"[eval] policy: {policy}" if policy
              else "[eval] no --policy: built-in sample move")
    allow_direct_connect = bool(direct_serve_control and relay_runner is None)
    allow_relay_connect = relay_runner is not None

    state, connect_arm, highlight, centered, prev_sel = "IDLE", False, 0, True, False
    prev = {b: False for b in E.SHORTCUT}
    show_connect = allow_direct_connect or allow_relay_connect
    menu = [m for m in E.MENU if show_connect or m != "CONNECT"] + ["VIEW", "RELAUNCH", "MARK A"]
    ...
    def enter(target):
        nonlocal state, judging, connect_arm, link
        if target == state:
            return
        policy_active = relay_runner is not None and relay_runner.active
        if not can_switch_state(state, target, policy_active):
            print(f"[eval] {target} ignored while relay policy is active", flush=True)
            return
        if state == "POLICY":
            policy_failed = relay_runner is not None and relay_runner.status == "error"
            if relay_runner is not None:
                relay_runner.stop()
            else:
                console.stop()
            judging = None if policy_failed else 0
            if policy_failed and recorder is not None and recorder.recording:
                recorder.event("policy_error", relay_runner.error or relay_runner.last)
                recorder.set_result("fail")
                recorder.stop()
        c.active = {k: False for k in c.active}
        state = target
        print(f"[eval] state = {state}", flush=True)
        if relay_runner is not None and should_release_command_link_for_state(state, link):
            link.close(shutdown=False)
            link = None
            print(f"[eval] command link closed for {state}", flush=True)
        if recorder is not None and recorder.recording:
            recorder.event("state", state)
        if state == "POLICY":
            if relay_runner is not None:
                released = release_teleop_link_for_policy(connect_arm, link)
                connect_arm, link = released.connect_arm, released.link
                if released.released:
                    print("[eval] teleop link released -> relay policy can own the serve",
                          flush=True)
            if recorder is not None:          # every policy run = one auto-recorded trial
                ...
                recorder.start()
                recorder.event("policy", relay_policy or policy or "sample")
    ...
    def choose(opt):
        ...
        if opt == "VIEW":
            ...
            if state == "POLICY" and relay_runner is not None:
                print("[eval] CONNECT ignored during relay policy", flush=True)
                return
            if relay_runner is not None:
                connect_arm = not connect_arm
                if connect_arm:
                    relay_runner.arm_on()
                    print("[eval] CONNECT on -> relay arm_on; command link only in TELEOP/GO_HOME",
                          flush=True)
                else:
                    if link is not None:
                        link.close(shutdown=False)
                        link = None
                    relay_runner.arm_off()
                    print("[eval] CONNECT off -> relay arm_off", flush=True)
                return
            if not allow_direct_connect:
            ...
            if connect_arm:
                ...
                link.connect_async()
                print("[eval] CONNECT on -> streaming sim joints to robot serve", flush=True)
            else:
                if link is not None:
                    link.close(shutdown=False)
                    link = None
            ...
            mujoco.mj_forward(m, d)
            state = "IDLE"
            print("[eval] RELAUNCH -> reset to home, IDLE", flush=True)
        else:
            enter(opt)
    ...
    try:
        while not c._stop_event.is_set():
            ...
            if state == "POLICY":
                if relay_runner is not None:
                    if relay_runner.done:
                        enter("IDLE")                      # -> enter() pops the verdict modal
                elif console._thread is not None and not console._thread.is_alive():
                    enter("IDLE")                          # -> enter() pops the verdict modal
            ...
            if state == "TELEOP":
                ...
                else:                                   # no clutch: keep sim target still
                ...
                if np.max(np.abs(home_qpos - d.qpos[:E.N_ARM])) < E.HOME_EPS:
                    ...
                    state = "IDLE"                 # no Mac command owner after home is reached
                    if relay_runner is not None and link is not None:
                        link.close(shutdown=False)
                        link = None
                        print("[eval] command link closed for IDLE", flush=True)
                    print("[eval] state = IDLE (home reached)", flush=True)

            # Gripper convention: trigger press TOGGLES open<->closed.
            ...
            if link is not None and link.status == "connected" and \
                    link.start_joints is not None and not getattr(link, "_sim_aligned", False):
                start = np.asarray(link.start_joints[:E.N_ARM], dtype=float)
                if len(start) == E.N_ARM:
                    d.qpos[:E.N_ARM] = start
                    d.ctrl[:E.N_ARM] = start
                    mujoco.mj_forward(m, d)
                    setattr(link, "_sim_aligned", True)
                    print(f"[connect] sim aligned to start_joints={np.round(start, 3)}",
                          flush=True)
            ...
            if connect_arm and relay_runner is not None:
                if should_attach_teleop_link(connect_arm, relay_runner.arm_status, state, link):
                    link = RobotLink(serve_host, serve_port)
                    link.connect_async()
                    print(f"[eval] relay serve ready -> linking {serve_host}:{serve_port}",
                          flush=True)
                elif link is not None and link.status == "error":
                    link.close(shutdown=False)
                    link = None
            ...
            if t0 - last_render >= render_period:
                if relay_runner is not None:
                    link_status = link.status if link is not None else relay_runner.arm_status
                else:
            ...
            LAT.note("loop_busy", time.monotonic() - t0)   # >20ms avg = loop misses 50Hz
        ...
        if link is not None:
            link.close(shutdown=relay_runner is None)  # direct mode owns shutdown; relay mode does not
        grabber.stop()
